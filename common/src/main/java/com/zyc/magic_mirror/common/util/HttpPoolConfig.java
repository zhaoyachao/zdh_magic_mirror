...
public class HttpPoolConfig {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HttpPoolConfig.class);
    ...
    private long retryInterval = 1000;        // 重试间隔时间(ms)
...
//            this.retryInterval = Long.parseLong(retryIntervalStr);
            ...
            logger.info("HTTP连接池配置已加载: {}", this);
        } catch (Exception e) {
            logger.warn("加载HTTP连接池配置失败，使用默认配置", e);
        }
    }
