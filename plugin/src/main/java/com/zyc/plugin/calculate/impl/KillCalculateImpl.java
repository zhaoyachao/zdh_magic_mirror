...
public class KillCalculateImpl extends BaseCalculate implements KillCalculate {
    ...
    public void run() {
        ...
        while (true){
            try{
                //获取要杀死的任务
                List<StrategyInstance> strategyInstanceList = strategyInstanceService.selectByStatus(new String[]{"kill"}, new String[]{"filter","shunt","touch","id_mapping","plugin", "manual_confirm"});
