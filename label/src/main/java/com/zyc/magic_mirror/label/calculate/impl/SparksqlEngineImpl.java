 ...
 * spark 引擎表达式实现
 */
public class SparksqlEngineImpl implements SqlEngine {
        ...
        if(!StringUtils.isEmpty(param_value)){
            ...
            if(unit.equalsIgnoreCase("day")){
                ...
                if(param_type!=null && (param_type.equalsIgnoreCase("date") || param_type.equalsIgnoreCase("timestamp"))){
                    ...
                    value=String.format("(unix_timestamp() - to_unix_timestamp(%s))/60 >="+start+" and (unix_timestamp() - to_unix_timestamp(%s))/60 <="+end, param_code,param_code);
                    if(start.contains("-") || end.contains("-")){
                        ...
                        value=String.format("(to_unix_timestamp(%s) - unix_timestamp())/60 >="+start+" and (to_unix_timestamp(%s) - unix_timestamp() )/60 <="+end, param_code,param_code);
                    }
                }else if(param_type!=null && !param_type.equalsIgnoreCase("ts")){
                    ...
                    value=String.format("(unix_timestamp() - %s)/60 >="+start+" and (unix_timestamp() - %s)/60 <="+end, param_code,param_code);
                    ...
                    if(start.contains("-") || end.contains("-")){
                        ...
                        value=String.format("(%s - unix_timestamp())/60 >="+start+" and (%s - unix_timestamp() )/60 <="+end, param_code,param_code);
                    }
                }
